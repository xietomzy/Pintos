...
### Algorithm:
* In `start_process`, get the first word using strtok_r() for the function`load`to use as its argument `filename`
* tokenize each argument and 
...
### Synchronization:
As long as each buffer points to a different address, there should be no synchronization issues for concurrent processes.
### Rationale: 
